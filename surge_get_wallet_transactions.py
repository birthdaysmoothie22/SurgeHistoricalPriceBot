			...
			for tx in result:
				...
				if tx["to"] == wallet_address and tx["from"] == contract_address:
					if 'allows_staking' in surge_tokens[surge_token] and surge_tokens[surge_token]['allows_staking']:
						...
						tx_type = "buy"
				elif 'mint_address' in surge_tokens[surge_token] and tx["to"] == wallet_address and tx["from"] == surge_tokens[surge_token]['mint_address']:
					tx_type = "buy"
				elif tx["to"] == wallet_address and tx["from"] != contract_address:
					tx_type = "received"
				elif tx["to"] == contract_address and tx["from"] == wallet_address:
					tx_type = "sell"
				elif tx["to"] != contract_address and tx["to"] != surge_fund_address and tx["from"] == wallet_address:
					tx_type = "sent"

				if len(tx_type):
					if 'decimals' in surge_tokens[surge_token] and surge_tokens[surge_token]['decimals'] > 0:
						tx["value"] = int(tx["value"]) / 10**surge_tokens[surge_token]['decimals']
